...
func (h *Handler) MakeWebhookHandler(ctx *gin.Context) {
	...
	if repoName == "" {
		...
		ctx.JSON(http.StatusBadRequest, dto.Error(http.StatusBadRequest, "无法获取仓库名", nil))
		return
	}
	ref := req.Ref
	if ref == "" {
		log.Printf("[webhook] 无法从 %s 仓库的请求中提取到分支信息", repoName)
		ctx.JSON(http.StatusBadRequest, dto.Error(http.StatusBadRequest, "无法获取分支信息", nil))
		return
	}
	...
	if !ok {
		...
		ctx.JSON(http.StatusNotFound, dto.Error(http.StatusNotFound, "未能找到存储库", nil))
		return
	}
	...
	if !utils.ValidateGitHubSignature(repoCfg.Secret, body, signature) {
		...
		ctx.JSON(http.StatusForbidden, dto.Error(http.StatusForbidden, "签名验证失败", nil))
		return
	}
	event := ctx.GetHeader("X-GitHub-Event")
	if event == "" {
		log.Printf("[webhook] 缺少 X-GitHub-Event 头")
		ctx.JSON(http.StatusBadRequest, dto.Error(http.StatusBadRequest, "缺少 X-GitHub-Event", nil))
		return
	}
	// 匹配规则
